...
namespace MLAgents
{
    ...
    public class RayPerception3D : RayPerception
    {
        ...
        public List<float> PerceiveMy(float rayDistance,
            float[] rayAngles, string[] detectableObjects,
            float startOffset, float endOffset)
        {
            perceptionBuffer.Clear();
            ...
            foreach (float angle in rayAngles)
            {
                ...
                float[] subList = new float[detectableObjects.Length];
                if (Physics.SphereCast(transform.position +
                                       new Vector3(0f, startOffset, 0f), 0.5f,
                    endPosition, out hit, rayDistance))
                {
                    for (int i = 0; i < detectableObjects.Length; i++)
                    {
                        if (hit.collider.gameObject.CompareTag(detectableObjects[i]))
                        {
                            subList[i] = hit.distance / rayDistance;
                            break;
                        }
                    }
                }
